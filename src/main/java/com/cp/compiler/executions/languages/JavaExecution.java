...
public class JavaExecution extends Execution {
    ...
    public Map<String, String> getParameters(String inputFileName) {
        ...
        String fileName = getSourceCodeFile().getOriginalFilename();
        String prefixName = fileName.substring(0, fileName.length() - 5); // remove ".java"
        String commandPrefix = "java -Djava.security.manager -Djava.security.policy=./security.policy " + prefixName;
        String executionCommand = inputFileName == null
                ? commandPrefix + "\n"
